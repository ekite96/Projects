...
const howlButton = document.querySelector('#submit');
howlButton.addEventListener('click', e => {
    const input = document.querySelector('#text-input');
    console.log(input.value);
    createHowl(input.value);
});

api.getCurrentUser().then(user => {
    fillHeaderHTML(user);
    api.getFollowingHowls(user.id).then(howls => {
        sortHowls(howls);
        updateHowls(howls);
    });
});
...
function createHowl(text) {
    api.newHowl(text).then(howls => {
